...
import { dbGetAccountByUsername, dbCreateAccount, dbUpdateAccount, dbLoadProgress, dbSaveProgress, dbInit } from '../lib/db';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Init DB (Supabase or localStorage fallback) on first mount
  useEffect(() => { dbInit(); initAccounts(); }, []);

  // Save to localStorage + sessionStorage + Supabase on every change
  useEffect(() => {
    if (user) {
  ...
  useEffect(() => { saveToLS('wwm_support', supportTickets); }, [supportTickets]);
  ...
  const register = async (username: string, password: string): Promise<string | null> => {
    ...
    const result = await dbCreateAccount(username.trim(), password);
    if ('error' in result) return result.error;

    const u: User = {
      id: result.id, email: '', name: result.username, picture: '', role: 'user',
    };
    setUser(u);
    saveToLS('wwm_user', u);

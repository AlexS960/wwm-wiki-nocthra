...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (ready) { dbSaveSiteData('guides', guides); markWrite('guides'); } }, [guides, ready]);
  useEffect(() => { if (ready) { dbSaveSiteData('site_settings', siteSettings); markWrite('settings'); } }, [siteSettings, ready]);
  useEffect(() => { if (ready) { dbSaveSiteData('chat', chatState); markWrite('chat'); } }, [chatState, ready]);
  useEffect(() => { if (ready) { dbSaveSiteData('wiki', wikiArticles); markWrite('wiki'); } }, [wikiArticles, ready]);
  useEffect(() => { if (ready) { dbSaveSiteData('support', supportTickets); markWrite('support'); } }, [supportTickets, ready]);
  useEffect(() => { if (ready) { dbSaveSiteData('pm', privateMessages); markWrite('pm'); } }, [privateMessages, ready]);

  // Track last write timestamps to prevent poll from overwriting recent local changes
  const lastWriteRef = { current: {} as Record<string, number> };
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (!cancelled) {
          // Only accept server data if we haven't written locally in the last 3 seconds
          const acceptServer = (key: string) => !lastWriteRef.current[key] || (now - lastWriteRef.current[key] > 3000);

          if (acceptServer('chat')) setChatState(prev => JSON.stringify(prev) === JSON.stringify(serverChat) ? prev : serverChat);
          if (acceptServer('support')) setSupportTickets(prev => JSON.stringify(prev) === JSON.stringify(serverSupport) ? prev : serverSupport);
          if (acceptServer('pm')) setPrivateMessages(prev => JSON.stringify(prev) === JSON.stringify(serverPM) ? prev : serverPM);
          if (acceptServer('guides') && serverGuides.length > 0) setGuides(prev => JSON.stringify(prev) === JSON.stringify(serverGuides) ? prev : serverGuides);
          if (acceptServer('settings') && serverSettings) setSiteSettings(prev => JSON.stringify(prev) === JSON.stringify(serverSettings) ? prev : serverSettings);
          if (acceptServer('wiki') && serverWiki.length > 0) setWikiArticles(prev => JSON.stringify(prev) === JSON.stringify(serverWiki) ? prev : serverWiki);

          // Users always sync from Supabase (doesn't conflict with local writes)
  ...
  // Helper to mark local writes
  const markWrite = (key: string) => { lastWriteRef.current[key] = Date.now(); };

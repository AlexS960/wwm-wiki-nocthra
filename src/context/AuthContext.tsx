...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (ready) { saveToLS('wwm_pm', privateMessages); dbSaveSiteData('pm', privateMessages); } }, [privateMessages, ready]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const [serverChat, serverSupport, serverPM, serverGuides, serverUsers, serverSettings, serverWiki] = await Promise.all([
        ...
        if (!cancelled) {
          ...
          if (serverUsers.length > 0) setRegisteredUsers(prev => JSON.stringify(prev) === JSON.stringify(serverUsers) ? prev : serverUsers);

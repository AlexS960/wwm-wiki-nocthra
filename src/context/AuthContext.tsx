...
import { dbGetAccountByUsername, dbCreateAccount, dbUpdateAccount, dbDeleteAccount, dbLoadProgress, dbSaveProgress, dbInit, dbLoadSiteData, dbSaveSiteData, dbSetOnline, dbGetOnlineStatuses } from '../lib/db';
...
const defaultSiteSettings: SiteSettings = {
  ...
  sections: [
    { id: 'guides', title: 'Гайды', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
    { id: 'weapons', title: 'Оружие', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
    { id: 'builds', title: 'Билды', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
    { id: 'sects', title: 'Секты', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
    { id: 'bosses', title: 'Боссы', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
    { id: 'mystic', title: 'Арты', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
    { id: 'map', title: 'Карта', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
    { id: 'cooking', title: 'Готовка', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
    { id: 'tips', title: 'Советы', maintenance: false, message: 'Раздел находится на технических работах. Попробуйте позже.' },
  ],
};
...
function initSiteSettings(): SiteSettings {
  ...
  if (saved.roles) {
    const ensurePerm = (roleId: string, perms: string[]) => {
      ...
      if (role) perms.forEach(p => { if (!role.permissions.includes(p)) role.permissions.push(p); });
    };
  ...
  if (!saved.sections) {
    saved.sections = defaultSiteSettings.sections;
  } else {
    // Ensure current sections exist
    defaultSiteSettings.sections.forEach(sec => {
      if (!saved.sections.find(s => s.id === sec.id)) saved.sections.push(sec);
    });
  }
  return saved;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Online heartbeat: save to server every 10s
  ...
  // Poll online statuses from server
  useEffect(() => {
    if (!ready) return;
    const poll = async () => {
      const statuses = await dbGetOnlineStatuses();
  ...
  // Auto-poll shared real-time-ish data: chat + support tickets
  useEffect(() => {
    if (!ready) return;
    ...
    const poll = async () => {
      try {
        const [serverChat, serverSupport] = await Promise.all([
          dbLoadSiteData<ChatState>('chat', { messages: [], mutedUsers: [] }),
          dbLoadSiteData<SupportTicket[]>('support', []),
        ]);
        ...
        if (!cancelled) {
          setChatState(prev => {
            ...
            const nextStr = JSON.stringify(serverChat);
            return prevStr === nextStr ? prev : serverChat;
          });
          setSupportTickets(prev => {
            const prevStr = JSON.stringify(prev);
            const nextStr = JSON.stringify(serverSupport);
            return prevStr === nextStr ? prev : serverSupport;
          });
        }
      } catch {}
    };
  ...
  const isUserOnline = (userId: string) => {
    // Check server-side online status first
    const serverTs = onlineStatuses[userId];
    if (serverTs && Date.now() - serverTs < 30000) return true;
    // Fallback to local
    const userEntry = registeredUsers.find(u => u.id === userId);
    if (!userEntry?.lastActiveAt) return false;
    return Date.now() - userEntry.lastActiveAt < 30000;
  };

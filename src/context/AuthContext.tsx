...
const defaultGuides: GuideArticle[] = [
  { id: 'guide-beginner', title: 'Полное руководство для новичков', category: 'Новичкам', difficulty: 'Начальный', readTime: '15 мин', summary: 'Всё, что нужно знать перед началом игры.', content: '## Начало игры\n\nПри создании персонажа вам предложат выбрать режим наведения и тип управления.\n\n## Система оружия\n\nВ игре нет жёстких классов.', authorName: 'Nocthra Wiki', updatedAt: '2025-11-15', icon: '📖' },
  { id: 'guide-combat', title: 'Боевая система: полный гайд', category: 'Бой', difficulty: 'Средний', readTime: '20 мин', summary: 'Разбор боевой системы.', content: '## Основы боя\n\nПарирование — ключевой навык.', authorName: 'Nocthra Wiki', updatedAt: '2025-11-15', icon: '⚔️' },
];
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [guides, setGuides] = useState<GuideArticle[]>(defaultGuides);
  ...
  useEffect(() => {
    (async () => {
      ...
      const [accounts, guidesData, settingsData, chatData, wikiData, supportData, pmData] = await Promise.all([
        dbListAccounts(),
        dbLoadSiteData<GuideArticle[]>('guides', defaultGuides),
      ...
      // Users from Supabase accounts
      const users: RegisteredUser[] = accounts.map(acc => ({
  ...
  useEffect(() => {
    ...
    const touch = () => {
      ...
      setRegisteredUsers(prev => prev.map(u => u.id === user.id ? { ...u, lastSeen: now, lastActiveAt: nowTs } : u));
      dbSetOnline(user.id);
    };
    touch();
    const interval = setInterval(touch, 10000);
  ...
  useEffect(() => {
    if (!ready) return;
    const poll = async () => { setOnlineStatuses(await dbGetOnlineStatuses()); };
    poll();
    const interval = setInterval(poll, 5000);
    return () => clearInterval(interval);
  }, [ready]);

  // ====== Save to Supabase whenever local state changes ======
  ...
  useEffect(() => { if (user && ready) dbSaveProgress(user.id, progress as unknown as Record<string, unknown>); }, [progress, user, ready]);

  // ====== Poll: read changes from OTHER users ======
  useEffect(() => {
    if (!ready) return;
    let cancelled = false;
    const poll = async () => {
      try {
        const [serverChat, serverSupport, serverPM, serverGuides, serverSettings, serverWiki, accounts] = await Promise.all([
          dbLoadSiteData<ChatState>('chat', { messages: [], mutedUsers: [] }),
          dbLoadSiteData<SupportTicket[]>('support', []),
          dbLoadSiteData<PrivateMessage[]>('pm', []),
        ...
        if (!cancelled) {
          // Update only if data actually changed (prevents flicker)
          setRegisteredUsers(prev => {
            const supUsers: RegisteredUser[] = accounts.map(acc => ({
              id: acc.id, email: '', name: acc.username, picture: acc.picture || '',
              gameNickname: acc.game_nickname || '', role: acc.role,
              joinedAt: acc.created_at ? new Date(acc.created_at).toLocaleDateString('ru-RU') : new Date().toLocaleDateString('ru-RU'),
              lastSeen: '—', isBanned: false,
            }));
            // Keep current user's online status
            for (const pu of prev) {
              const su = supUsers.find(u => u.id === pu.id);
              if (su) { su.lastActiveAt = pu.lastActiveAt; su.lastSeen = pu.lastSeen; }
            }
            // Always include current user
            if (userRef.current && !supUsers.find(u => u.id === userRef.current!.id)) {
              const cu = userRef.current;
              supUsers.push({ id: cu.id, email: '', name: cu.name, picture: cu.picture || '', gameNickname: cu.gameNickname || '', role: cu.role, joinedAt: new Date().toLocaleDateString('ru-RU'), lastSeen: new Date().toLocaleDateString('ru-RU'), lastActiveAt: Date.now(), isBanned: false });
            }
            return JSON.stringify(prev) === JSON.stringify(supUsers) ? prev : supUsers;
          });

          const eq = (a: string, b: string) => a === b;
          const serverChatS = JSON.stringify(serverChat);
          const serverSupportS = JSON.stringify(serverSupport);
          const serverPMS = JSON.stringify(serverPM);
          const serverGuidesS = JSON.stringify(serverGuides);
          const serverSettingsS = serverSettings ? JSON.stringify(serverSettings) : '';
          const serverWikiS = JSON.stringify(serverWiki);

          setChatState(prev => { const s = JSON.stringify(prev); return eq(s, serverChatS) ? prev : serverChat; });
          setSupportTickets(prev => { const s = JSON.stringify(prev); return eq(s, serverSupportS) ? prev : serverSupport; });
          setPrivateMessages(prev => { const s = JSON.stringify(prev); return eq(s, serverPMS) ? prev : serverPM; });
          if (serverGuides.length > 0) setGuides(prev => eq(JSON.stringify(prev), serverGuidesS) ? prev : serverGuides);
          if (serverSettings) setSiteSettings(prev => eq(JSON.stringify(prev), serverSettingsS) ? prev : serverSettings);
          if (serverWiki.length > 0) setWikiArticles(prev => eq(JSON.stringify(prev), serverWikiS) ? prev : serverWiki);
        }
      } catch {}
    };
  ...
  // ====== Auth ======
  const loginWithPassword = async (username: string, password: string, _remember: boolean): Promise<string | null> => {
    ...
    if (p) setProgress(p as unknown as UserProgress);
    return null;
  };
  ...
  const register = async (username: string, password: string, gameNickname = ''): Promise<string | null> => {
    ...
    const u: User = { id: result.id, email: '', name: result.username, picture: '', gameNickname: result.game_nickname || '', role: 'user' };
    setUser(u);
    return null;
  };

  const logout = async () => {
    if (user) await dbSaveProgress(user.id, progress as unknown as Record<string, unknown>);
    setUser(null); setProgress(defaultProgress);
  };

  // ====== Helpers ======
  const setUserRole = (role: UserRole) => { if (user) { const u = { ...user, role }; setUser(u); dbUpdateAccount(user.id, { role }); } };
  const isEditor = () => !!user && (user.role === 'editor' || user.role === 'admin');
  const isAdmin = () => !!user && user.role === 'admin';
  const adminSetUserRole = (userId: string, role: UserRole) => { if (!isAdmin()) return; setRegisteredUsers(prev => prev.map(u => u.id === userId ? { ...u, role } : u)); dbUpdateAccount(userId, { role }); };
  ...
  const isUserOnline = (userId: string) => {
    const serverTs = onlineStatuses[userId];
    if (serverTs && Date.now() - serverTs < 30000) return true;
    const ue = registeredUsers.find(u => u.id === userId);
    return ue?.lastActiveAt ? Date.now() - ue.lastActiveAt < 30000 : false;
  };
  ...
  const updateUserPicture = (picture: string) => { if (!user) return; const u = { ...user, picture }; setUser(u); setRegisteredUsers(prev => prev.map(r => r.id === user.id ? { ...r, picture } : r)); dbUpdateAccount(user.id, { picture }); };
  const updateUserGameNickname = (gameNickname: string) => { if (!user) return; const u = { ...user, gameNickname }; setUser(u); setRegisteredUsers(prev => prev.map(r => r.id === user.id ? { ...r, gameNickname } : r)); dbUpdateAccount(user.id, { game_nickname: gameNickname }); };
  const addGuide = (guide: Omit<GuideArticle, 'id' | 'authorName' | 'updatedAt'>) => { if (!isEditor()) return; setGuides(prev => [{ ...guide, id: 'guide_' + Date.now(), authorName: user!.name, updatedAt: new Date().toLocaleDateString('ru-RU') }, ...prev]); };
  ...
  const sendPrivateMessage = (toId: string, toName: string, text: string) => { if (!user || !text.trim()) return; setPrivateMessages(prev => [...prev.slice(-500), { id: 'pm_' + Date.now(), fromId: user.id, fromName: user.name, toId, toName, text: text.trim(), timestamp: Date.now(), read: false }]); };
  const markPMRead = (fromId: string) => { setPrivateMessages(prev => prev.map(m => m.fromId === fromId && m.toId === user?.id && !m.read ? { ...m, read: true } : m)); };

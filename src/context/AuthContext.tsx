...
export function AuthProvider({children}:{children:ReactNode}) {
  ...
  const applySiteDataKey = useCallback((key: string, value: unknown) => {
    switch (key) {
      ...
      case 'pm': setPrivateMessages(asArray<PrivateMessage>(value)); break;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setDbSaveError('Не удалось подключиться к Supabase. Проверьте VITE_SUPABASE_URL и VITE_SUPABASE_ANON_KEY в .env');
        setIsLoading(false);
        return;
      }
      ...
      setChatState(c); setWikiArticles(asArray<WikiArticle>(w)); setSupportTickets(asArray(t)); setPrivateMessages(asArray(pm));
  ...
  const value: Ctx = {
    ...
    markPMRead: async (partnerId) => {
      if (!user) return;
      const prev = pmRef.current;
      const next = prev.map(x => x.fromId === partnerId && x.toId === user.id ? { ...x, read: true } : x);
      setPrivateMessages(next);
      await persist('pm', next);
    },

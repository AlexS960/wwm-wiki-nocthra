...
import { trimPrivateMessages, type PrivateMessage } from '../lib/pm';

export type { PrivateMessage };

const MAX_GUIDE_VERSIONS = 15;
const MAX_CHAT_MESSAGES = 350;
...
export function AuthProvider({children}:{children:ReactNode}) {
  ...
  const applySiteDataKey = useCallback((key: string, value: unknown) => {
    switch (key) {
      case 'guides': setGuides(asArray<GuideArticle>(value)); break;
      ...
      case 'wiki': setWikiArticles(asArray<WikiArticle>(value)); break;
      case 'support': setSupportTickets(asArray<SupportTicket>(value)); break;
      case 'pm': setPrivateMessages(asArray<PrivateMessage>(value)); setPmLoaded(true); break;
      case 'guild': setGuild(value as GuildData); break;
      case 'discord_url': setDiscordUrl(typeof value === 'string' ? value : String(value)); break;
      case 'site_news': setSiteNews(asArray<SiteNewsItem>(value)); break;
      case 'guide_comments': setGuideComments(asArray<GuideComment>(value)); break;
      case 'guide_versions': setGuideVersions(asArray<GuideVersion>(value)); break;
    }
  }, [mergeSiteSettings]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [accs, g, s, c, w, t, guildData, discord, news, comments, versions] = await Promise.all([
      ...
      setGuides(asArray<GuideArticle>(g)); setSiteSettings(mergeSiteSettings(s as SiteSettings | null));
      setChatState(c); setWikiArticles(asArray<WikiArticle>(w)); setSupportTickets(asArray(t));
      void dbLoadSiteData('pm', []).then(pm => {
        if (!active) return;
        setPrivateMessages(asArray<PrivateMessage>(pm));
        setPmLoaded(true);
      });
      setGuild(guildData); setDiscordUrl(discord);
      setSiteNews(asArray<SiteNewsItem>(news));
      setGuideComments(asArray<GuideComment>(comments));
      setGuideVersions(asArray<GuideVersion>(versions));
  ...
  const value: Ctx = {
    ...
    sendPrivateMessage: async (toId, text) => {
      ...
      const next = trimPrivateMessages([...prev, msg]);
    ...
    markPMRead: async (partnerId) => {
      ...
      const next = prev.map(x => {
        if (x.fromId === partnerId && x.toId === user.id && !x.read) {
          changed = true;
          return { ...x, read: true };
        }
        return x;
      ...
      if (!changed) return;
      setPrivateMessages(next);
      await persist('pm', trimPrivateMessages(next));
    },
    deletePrivateMessageForMe: async (messageId) => {
      ...
      if (!msg) return null;
      const uid = user.id;
      if (msg.hiddenFor?.includes(uid)) return null;
      const next = trimPrivateMessages(prev.map(m => {
        if (m.id !== messageId) return m;
        return { ...m, hiddenFor: [...(m.hiddenFor || []), uid] };
      }));
      setPrivateMessages(next);
      const err = await persist('pm', next);
      if (err) { setPrivateMessages(prev); return err; }
      return null;
    },
    deletePrivateMessageForAll: async (messageId) => {
      ...
      const prev = pmRef.current;
      const msg = prev.find(m => m.id === messageId);
      if (!msg || msg.fromId !== user.id) return 'Можно удалить только свои сообщения';
      if (msg.deletedForAll) return null;
      if (Date.now() - msg.timestamp > 48 * 60 * 60 * 1000) {
        return 'Удалить у всех можно в течение 48 часов после отправки';
      }
      const next = trimPrivateMessages(prev.map(m =>

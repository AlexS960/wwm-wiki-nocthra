...
interface Ctx {
  ...
  guild: GuildData; discordUrl: string; updateGuild: (g: GuildData) => void; updateDiscordUrl: (url: string) => void;
...
export function AuthProvider({children}:{children:ReactNode}) {
  ...
  const mapAccounts = useCallback((accs: Awaited<ReturnType<typeof dbListAccounts>>) =>
    accs.map(a => ({id:a.id,email:'',name:a.username,picture:a.picture||'',gameNickname:a.game_nickname||'',role:a.role,joinedAt:a.created_at,lastSeen:'—',isBanned:false})),
  []);

  const applySiteDataKey = useCallback((key: string, value: unknown) => {
    switch (key) {
      case 'guides': setGuides(value as GuideArticle[]); break;
      case 'site_settings': if (value) setSiteSettings(value as SiteSettings); break;
  ...
  useEffect(() => {
    ...
    (async () => {
      await dbInit();
      const [accs, g, s, c, w, t, pm, guildData, discord] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const fallback = setInterval(async () => {
      ...
      try {
        const [c, t, pm, g, s, w, guildData, discord, accs] = await Promise.all([
        ...
        setGuides(g); if (s) setSiteSettings(s); setWikiArticles(w || []);
        setGuild(guildData); setDiscordUrl(discord);
        setRegisteredUsers(mapAccounts(accs));
      } catch {}
    }, 15000);
  ...
  const value: Ctx = {
    ...
    createTicket:(s,m)=>{if(!user)return; saveSite('support', prev=>[...prev,{id:'t'+Date.now(),userId:user.id,userName:user.name,subject:s,message:m,status:'open' as const,createdAt:new Date().toISOString(),replies:[]}],setSupportTickets);},
    replyToTicket:(id,m)=>saveSite('support', prev=>prev.map(x=>x.id===id?{...x,status:'answered' as const,replies:[...x.replies,{id:'r'+Date.now(),authorName:user?.name||'',authorRole:'',message:m,createdAt:new Date().toISOString()}]}:x),setSupportTickets),
    ...
    sendMessage:(text)=>{if(!user)return; saveSite('chat', prev=>({...prev,messages:[...prev.messages,{id:'m'+Date.now(),userId:user.id,userName:user.name,userRole:user.role,text,timestamp:Date.now()}]}),setChatState);},
    ...
    sendPrivateMessage:(toId,toName,text)=>{if(!user)return; saveSite('pm', prev=>[...prev,{id:'p'+Date.now(),fromId:user.id,fromName:user.name,toId,toName,text,timestamp:Date.now(),read:false}],setPrivateMessages);},
    markPMRead:(id)=>saveSite('pm', prev=>prev.map(x=>x.fromId===id&&x.toId===user?.id?{...x,read:true}:x),setPrivateMessages),
  };

...
interface AuthContextType {
  ...
  loginWithPassword: (u: string, p: string, r: boolean) => Promise<string | null>; register: (u: string, p: string, gn?: string) => Promise<string | null>; logout: () => void;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  const [user, setUser] = useState<User | null>(() => {
    try {
      const saved = localStorage.getItem('wwm_user') || sessionStorage.getItem('wwm_user');
  ...
  useEffect(() => {
    (async () => {
      ...
      if (userRef.current) {
        const pr = await db.progress.load(userRef.current.id);
  ...
  const value = useMemo(() => {
    const contextValue = {
      ...
      loginWithPassword: async (u: string, p: string, remember: boolean) => {
        const acc = await db.accounts.get(u);
        if (!acc || acc.password_hash !== p) return 'Ошибка';
        const userData: User = { id: acc.id, email: '', name: acc.username, picture: acc.picture || '', gameNickname: acc.game_nickname || '', role: acc.role };
        setUser(userData);
        if (remember) localStorage.setItem('wwm_user', JSON.stringify(userData));
        else sessionStorage.setItem('wwm_user', JSON.stringify(userData));
        const pr = await db.progress.load(acc.id);
        if (pr) setProgress(pr as any);
        return null;
      },
      register: async (u: string, p: string, gn = '') => {
        try {
          const res = await db.accounts.create(u, p, gn);
          if ('error' in res) return res.error;
          const userData: User = { id: res.id, email: '', name: res.username, picture: '', gameNickname: res.game_nickname || '', role: 'user' };
          setUser(userData);
          localStorage.setItem('wwm_user', JSON.stringify(userData));
          return null;
      ...
      updateProgress: (u: any) => { const n = { ...progress, ...u }; setProgress(n); if (userRef.current) db.progress.save(userRef.current.id, n); },
      toggleFavoriteWeapon: (id: string) => contextValue.updateProgress({ favoriteWeapons: progress.favoriteWeapons.includes(id) ? progress.favoriteWeapons.filter(x => x !== id) : [...progress.favoriteWeapons, id] }),
      toggleFavoriteSect: (id: string) => contextValue.updateProgress({ favoriteSects: progress.favoriteSects.includes(id) ? progress.favoriteSects.filter(x => x !== id) : [...progress.favoriteSects, id] }),
      toggleCompletedGuide: (id: string) => contextValue.updateProgress({ completedGuides: progress.completedGuides.includes(id) ? progress.completedGuides.filter(x => x !== id) : [...progress.completedGuides, id] }),
      toggleVisitedRegion: (id: string) => contextValue.updateProgress({ visitedRegions: progress.visitedRegions.includes(id) ? progress.visitedRegions.filter(x => x !== id) : [...progress.visitedRegions, id] }),
      addNote: (t: string, c: string) => contextValue.updateProgress({ notes: [{ id: 'n'+Date.now(), title: t, content: c, date: new Date().toLocaleDateString() }, ...progress.notes] }),
      deleteNote: (id: string) => contextValue.updateProgress({ notes: progress.notes.filter(x => x.id !== id) }),
      setSelectedBuild: (id: string | null) => contextValue.updateProgress({ selectedBuild: id }),
      updateUserPicture: (p: string) => { if (userRef.current) { setUser({...userRef.current, picture:p}); db.accounts.update(userRef.current.id, {picture:p}); } },
      updateUserGameNickname: (n: string) => { if (userRef.current) { setUser({...userRef.current, gameNickname:n}); db.accounts.update(userRef.current.id, {game_nickname:n}); } },
      ...
      adminBanUser: (_id: string, _b: boolean) => {}, 
      adminDeleteUser: (id: string) => db.accounts.remove(id),
      ...
      deleteRole: (id: string) => sync('site_settings', { ...siteSettings, roles: siteSettings.roles.filter(r => r.id !== id) }, setSiteSettings),
      ...
      createTicket: (subject: string, message: string) => { if (!user) return; const t: SupportTicket = { id: 't'+Date.now(), userId: user.id, userName: user.name, subject, message, status: 'open', createdAt: new Date().toISOString(), replies: [] }; sync('support', [...supportTickets, t], setSupportTickets); },

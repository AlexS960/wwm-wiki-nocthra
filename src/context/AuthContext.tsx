...
export function AuthProvider({children}:{children:ReactNode}) {
  ...
  const refreshPrivateMessages = useCallback(async () => {
    ...
    try {
      ...
      const msgs = await pmLoadInbox(user.id);
  ...
  useEffect(() => {
    ...
    const unsubPm = user ? subscribePmMessages(() => { void refreshPrivateMessages(); }) : () => {};
  ...
  }, [isLoading, applySiteDataKey, mapAccounts, user?.id, refreshPrivateMessages]);

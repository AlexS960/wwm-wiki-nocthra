...
import { findSiteGuildmaster } from '../lib/guildRegistry';
...
function GuildInfoPopup({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const { guild, updateGuild, hasPermission, registeredUsers, ensureAccountsLoaded, siteSettings } = useAuth();
  ...
  const siteGuildmasterName = useMemo(() => {
    const gm = findSiteGuildmaster(registeredUsers, siteSettings.roles);
    return gm ? getDisplayName(gm) : '—';
  }, [registeredUsers, siteSettings.roles]);

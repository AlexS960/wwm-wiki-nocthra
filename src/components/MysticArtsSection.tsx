...
export default function MysticArtsSection() {
  ...
  const items = useMemo(() => (
    Array.isArray(sectionOverrides.mystic) ? (sectionOverrides.mystic as typeof mysticArts) : mysticArts
  ), [sectionOverrides]);
  const persistItems = (next: typeof mysticArts) => {
    updateSiteSettings({ sectionOverrides: { ...sectionOverrides, mystic: next } });
  };

...
export default function LifeSkillsSection() {
  const [items, setItems] = useState(lifeSkills);
            ...
            <div key={skill.name}
              ...
              {canManage && (
                <div className="flex justify-end gap-1 mb-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => setEditName(skill.name)} className="px-1.5 py-0.5 rounded text-[10px] text-gold-300 border border-gold-400/30 hover:bg-gold-400/10 cursor-pointer inline-flex items-center gap-1">
                    <Edit3 className="w-3 h-3" /> Ред.
                  </button>
                  <button
                    onClick={() => {
                      if (!confirm('Удалить навык?')) return;
                      setItems(prev => prev.filter(x => x.name !== skill.name));
                    }}
                    className="px-1.5 py-0.5 rounded text-[10px] text-crimson-300 border border-crimson-400/30 hover:bg-crimson-400/10 cursor-pointer inline-flex items-center gap-1"
                  >
      ...
      {editingItem && lifeConfig && (
        <SectionEditorModal
          ...
          onSave={(values: SectionEditorValues) => {
            ...
            setItems(prev => prev.map((s: LifeSkill) => (

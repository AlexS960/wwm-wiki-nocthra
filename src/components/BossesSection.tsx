              ...
              <div className="flex items-start justify-between mb-3">
                ...
                {canManage && (
                  <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                    <button onClick={() => setEditId(boss.id)} className="px-2 py-1 rounded-lg text-[10px] text-gold-300 border border-gold-400/30 hover:bg-gold-400/10 cursor-pointer inline-flex items-center gap-1">
                      <Edit3 className="w-3 h-3" /> Редактировать
                    </button>
                    <button
                      onClick={() => {
                        if (!confirm('Удалить карточку босса?')) return;
                        setItems(prev => prev.filter(x => x.id !== boss.id));

...
export default function BossesSection() {
  ...
  const items = useMemo(() => (
    Array.isArray(sectionOverrides.bosses) ? (sectionOverrides.bosses as typeof bosses) : bosses
  ), [sectionOverrides]);
  const persistItems = (next: typeof bosses) => {
    updateSiteSettings({ sectionOverrides: { ...sectionOverrides, bosses: next } });
  };

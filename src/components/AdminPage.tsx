...
export default function AdminPage({ onBack }: AdminPageProps) {
  ...
  useEffect(() => {
    void Promise.all([ensureWikiLoaded(), ensureSupportLoaded(), ensureGuideMetaLoaded()]);
    ...
    <div className="space-y-6 animate-fadeIn max-w-2xl">
      <div className="bg-ink-800/50 border border-ink-700/30 rounded-xl p-5 space-y-4">
        ...
        <p className="text-ink-500 text-xs mt-2">
          Скриншоты только через Supabase Storage (<code className="text-gold-400/80">storage-setup.sql</code>). Base64 в базе не сохраняется — это снижает egress.
        </p>
        <button
          type="button"
          disabled={purging}

...
import {
  ...
  Edit3, AlertTriangle, Check, Megaphone, Eye, EyeOff, Tag, Palette, Wrench
} from 'lucide-react';

interface AdminPageProps { onBack: () => void; }

type Tab = 'users' | 'roles' | 'sections' | 'guides' | 'settings' | 'announcements';
...
function UsersPanel() {
  ...
  const mergedUsers = accounts.map(acc => {
...
function SectionsPanel() {
  ...
  useEffect(() => {
    ...
    setSections(merged);
    if (merged.length !== current.length) updateSiteSettings({ sections: merged });
  ...
  const addSection = () => {
    ...
    const next = [...sections, { id, title: newSectionTitle.trim(), maintenance: true, message: 'Раздел находится на технических работах. Попробуйте позже.' }];
    saveSections(next);
    setNewSectionId('');
    setNewSectionTitle('');
  };
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      <div className="bg-ink-800/50 border border-ink-700/30 rounded-xl p-4">
        <h3 className="text-gold-400 font-semibold text-sm flex items-center gap-2 mb-3"><Wrench className="w-4 h-4" /> Управление разделами сайта</h3>
        <p className="text-ink-400 text-xs mb-4">Включайте технические работы для любых разделов из вкладки «Информация». Для новых разделов можно заранее создать заглушку.</p>
        <div className="grid sm:grid-cols-[1fr_1fr_auto] gap-2">
          <input value={newSectionTitle} onChange={e => setNewSectionTitle(e.target.value)} placeholder="Название нового раздела"
            className="bg-ink-800 border border-ink-700/50 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-ink-500 focus:outline-none focus:border-gold-400/50" />
          <input value={newSectionId} onChange={e => setNewSectionId(e.target.value)} placeholder="ID (например: quests)"
            className="bg-ink-800 border border-ink-700/50 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-ink-500 focus:outline-none focus:border-gold-400/50" />
          <button onClick={addSection} className="px-4 py-2.5 bg-gold-400/20 text-gold-400 rounded-lg text-sm font-medium hover:bg-gold-400/30 cursor-pointer">Добавить</button>
        </div>
      </div>
...
function SettingsPanel() {
  const { siteSettings, updateSiteSettings } = useAuth();
  const [name, setName] = useState(siteSettings.siteName);
  const [desc, setDesc] = useState(siteSettings.siteDescription);
  const [discord, setDiscord] = useState(siteSettings.discordUrl);
  const [saved, setSaved] = useState(false);

  const handleSave = () => { updateSiteSettings({ siteName: name, siteDescription: desc, discordUrl: discord }); setSaved(true); setTimeout(() => setSaved(false), 2000); };

  return (
    <div className="space-y-6 animate-fadeIn max-w-2xl">
      <div className="bg-ink-800/50 border border-ink-700/30 rounded-xl p-5 space-y-4">
        <h3 className="text-gold-400 font-semibold text-sm flex items-center gap-2"><Settings className="w-4 h-4" /> Информация о сайте</h3>
        <div><label className="text-ink-400 text-xs mb-1 block">Название</label><input value={name} onChange={e => setName(e.target.value)} className="w-full bg-ink-800 border border-ink-700/50 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-gold-400/50" /></div>
        <div><label className="text-ink-400 text-xs mb-1 block">Описание</label><input value={desc} onChange={e => setDesc(e.target.value)} className="w-full bg-ink-800 border border-ink-700/50 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-gold-400/50" /></div>
        <div><label className="text-ink-400 text-xs mb-1 block">Discord URL</label><input value={discord} onChange={e => setDiscord(e.target.value)} className="w-full bg-ink-800 border border-ink-700/50 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-gold-400/50" /></div>

...
import {
  ...
  Edit3, AlertTriangle, Check, Megaphone, Eye, EyeOff, Tag, Palette
} from 'lucide-react';

interface AdminPageProps { onBack: () => void; }

type Tab = 'users' | 'roles' | 'guides' | 'settings' | 'announcements';
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatBox label="Всего" value={registeredUsers.length} icon="👤" />
        {siteSettings.roles.map(role => (
          <StatBox key={role.id} label={role.displayName} value={registeredUsers.filter(u => u.role === role.id).length}
              ...
              return (
                <div key={u.id} className={`grid grid-cols-1 md:grid-cols-12 gap-2 px-4 py-3 items-center ${u.isBanned ? 'opacity-50 bg-crimson-400/5' : ''}`}>
                  ...
                  <div className="col-span-3 flex items-center gap-2">
                    <select value={u.role} onChange={e => adminSetUserRole(u.id, e.target.value as UserRole)}
                  ...
                  <div className="col-span-3 flex items-center gap-1 justify-end">
                    <button onClick={() => adminBanUser(u.id, !u.isBanned)} title={u.isBanned ? 'Разбанить' : 'Забанить'}
      ...
      {deleteConfirm && <ConfirmModal title="Удалить пользователя?" message="Это действие нельзя отменить."
        onConfirm={() => { adminDeleteUser(deleteConfirm); setDeleteConfirm(null); }} onCancel={() => setDeleteConfirm(null)} />}
    </div>
  );
}

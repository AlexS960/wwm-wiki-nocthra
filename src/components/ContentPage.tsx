...
export default function ContentPage({ pageId, onBack }: ContentPageProps) {
  ...
  return (
    <div className="min-h-screen bg-ink-900 text-ink-100 pt-16 md:pt-20">
      ...
      {/* Static content */}
      {pageId === 'weapons' && <><WeaponsSection /><WikiEditor sectionId="weapons" /></>}
      {pageId === 'builds' && <><BuildsSection /><WikiEditor sectionId="builds" /></>}
      {pageId === 'sects' && <><SectsSection /><WikiEditor sectionId="sects" /></>}
      {pageId === 'bosses' && <><BossesSection /><WikiEditor sectionId="bosses" /></>}
      {pageId === 'mystic' && <><MysticArtsSection /><WikiEditor sectionId="mystic" /></>}
      {pageId === 'map' && <InteractiveMap />}
      {pageId === 'cooking' && <><CookingSection /><WikiEditor sectionId="cooking" /></>}
      {pageId === 'tips' && <><TipsSection /><WikiEditor sectionId="tips" /></>}

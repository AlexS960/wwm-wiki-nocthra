...
import { Search, Sparkles, BookOpen, TrendingUp, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react';
import {
  innerWays,
  innerPathOrder,
  getPathMeta,
...
function InfoBlock({ title, body, defaultOpen = true }: { title: string; body: string; defaultOpen?: boolean }) {
  ...
  return (
    <div className="bg-ink-800/50 border border-ink-700/30 rounded-xl overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left hover:bg-ink-700/30 transition-colors cursor-pointer"
      >
      ...
      {open && (
        <div className="px-4 pb-4 space-y-2 border-t border-ink-700/30 pt-3">
          {paragraphs.map((p, i) => (
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="font-serif font-bold text-white text-base sm:text-lg">{item.nameEn}</h3>
        ...
        {!expanded && (
          <p className="text-ink-400 text-xs mt-2 line-clamp-2">{item.effect}</p>
        )}
      </button>
      ...
      {expanded && (
        <div className="px-4 sm:px-5 pb-4 sm:pb-5 space-y-3 border-t border-ink-700/30 pt-3">
          <div>
            ...
            <p className="text-ink-200 text-sm leading-relaxed">{item.effect}</p>
          </div>
          <div>
            ...
            <p className="text-ink-300 text-sm leading-relaxed">{item.howToGet}</p>
          </div>
        </div>
      )}
    </article>
...
export default function InnerPathSection() {
  ...
  const filtered = useMemo(() => {
    ...
    return innerWays.filter(w => {
      ...
      if (!q) return true;
      const hay = `${w.nameEn} ${w.pathEn} ${w.effect} ${w.howToGet} ${getPathMeta(w.pathEn).labelRu}`.toLowerCase();
        ...
        <div className="mb-10">
          ...
          <div className="space-y-3 mb-4">
            {innerPathUpgradeRu.map((block, i) => (
              <InfoBlock key={i} title={`${i + 1}. ${block.title}`} body={block.body} defaultOpen={i === 0} />
            ))}
          </div>

...
import { Search, Sparkles, BookOpen, TrendingUp, ChevronDown, Edit3, Trash2 } from 'lucide-react';
import {
  innerWays,
  innerPathOrder,
  getPathMeta,
...
export default function InnerPathSection() {
  ...
  const items = useMemo(() => (
    Array.isArray(sectionOverrides.innerpath) ? (sectionOverrides.innerpath as typeof innerWays) : innerWays
  ), [sectionOverrides]);
  const persistItems = (next: typeof innerWays) => {
    updateSiteSettings({ sectionOverrides: { ...sectionOverrides, innerpath: next } });
  };
  const innerConfig = sectionEditorConfigs.innerpath;
  const editingItem = editId ? items.find(x => x.id === editId) : null;

  const paths = useMemo(() => {
    const set = new Set(items.map(w => w.pathEn));
  ...
  }, [items, filterPath, search]);

  const countByPath = useMemo(() => {
    const map: Record<string, number> = {};
    for (const w of items) map[w.pathEn] = (map[w.pathEn] ?? 0) + 1;
    return map;
  }, [items]);
      ...
      {editingItem && innerConfig && (
        <SectionEditorModal
          ...
          initial={{
            ...
            icon: getPathMeta(editingItem.pathEn).icon,

        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <button onClick={() => onNavigate('home')} className="flex items-center gap-2 group cursor-pointer">
            ...
            <div className="flex flex-col">
              <span className="font-serif text-gold-400 text-lg md:text-xl font-bold leading-tight group-hover:text-gold-300 transition-colors">
                WWM Wiki
              </span>
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {infoDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-ink-900/98 backdrop-blur-md border border-gold-700/30 
                              rounded-xl shadow-2xl shadow-black/40 overflow-hidden animate-fadeIn z-50">
                  <div className="py-2">
                    {infoItems.map(item => (
                      <button
                        key={item.id}
                        onClick={() => { onNavigate(item.id); setInfoDropdownOpen(false); }}
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setMobileOpen(!mobileOpen)}
              className="lg:hidden p-2 text-ink-200 hover:text-gold-400 transition-colors cursor-pointer">
      ...
      {mobileOpen && (
        <nav className="lg:hidden bg-ink-900/98 backdrop-blur-md border-t border-gold-700/20 animate-fadeIn">
          <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1">
            <MobileNavButton label="🏠 Главная" active={activeSection === 'home'}
              onClick={() => { onNavigate('home'); setMobileOpen(false); }} />

            <div className="px-4 pt-3 pb-1 text-[10px] text-ink-500 uppercase tracking-widest font-semibold">Информация</div>
            {infoItems.map(item => (
              <MobileNavButton key={item.id} label={`${item.icon} ${item.label}`} active={activeSection === item.id}
                onClick={() => { onNavigate(item.id); setMobileOpen(false); }} />
            ))}

            <div className="h-px bg-ink-700/50 my-1" />
            <MobileNavButton label="❓ FAQ" active={activeSection === 'faq'}
              onClick={() => { onNavigate('faq'); setMobileOpen(false); }} />
              ...
              {user ? (
                <button onClick={() => { onProfileClick(); setMobileOpen(false); }}
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-xs text-ink-400">Открыть профиль</div>
                  </div>
                </button>
              ) : (
                <button onClick={() => { onLoginClick(); setMobileOpen(false); }}
                  ...
                  <LogIn className="w-4 h-4" /><span>Войти в профиль</span>
                </button>
              )}
            </div>
          </div>
...
function MobileNavButton({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick}
      className={`px-4 py-3 rounded-lg text-left text-sm font-medium cursor-pointer ${
        active ? 'text-gold-400 bg-gold-400/10' : 'text-ink-200 hover:text-gold-300 hover:bg-white/5'
      }`}>
      {label}
    </button>

...
import { Menu, X, Scroll, User, LogIn } from 'lucide-react';
...
export default function Header({ activeSection, onNavigate, onLoginClick, onProfileClick }: HeaderProps) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? 'bg-ink-900/95 backdrop-blur-md shadow-lg shadow-black/30 border-b border-gold-700/30' : 'bg-transparent'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            <NavButton label="WWM-Wiki" active={isWikiPageActive} onClick={() => onNavigate('wwmwiki')} />
            <NavButton label="Список пользователей" active={activeSection === 'users'} onClick={() => onNavigate('users')} />
      ...
      {mobileOpen && (
        <>
          ...
          <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1 pb-8">
            ...
            <MobileNavButton label="☯️ WWM-Wiki" active={isWikiPageActive} onClick={() => { onNavigate('wwmwiki'); setMobileOpen(false); }} />

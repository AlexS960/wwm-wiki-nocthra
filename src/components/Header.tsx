...
import { Menu, X, Scroll, User, LogIn, House, BookOpenText, Users, CircleHelp, Crown } from 'lucide-react';
...
export default function Header({ activeSection, onNavigate, onLoginClick, onProfileClick, showAdminLink }: HeaderProps) {
      ...
      <div className="w-full px-3 sm:px-4 lg:px-3 xl:px-4 2xl:px-6">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            {showAdminLink && (
              <NavButton icon={<Crown className="w-4 h-4" />} label="Админ" active={activeSection === 'admin'} onClick={() => onNavigate('admin')} accent="purple" />
            )}
          </nav>
        ...
        <>
          ...
          >
            <div className="w-full px-4 py-4 flex flex-col gap-1 pb-8">
              ...
              {showAdminLink && (
                <MobileNavButton label="👑 Панель управления" active={activeSection === 'admin'} onClick={() => { onNavigate('admin'); setMobileOpen(false); }} />
              )}

...
export default function RiddlesSection() {
  ...
  const masters = useMemo(
    () => mergeRiddleMasters(
      wikiArticles,
      hiddenIds,
      siteSettings.parsedContent?.riddles?.masters?.length
        ? siteSettings.parsedContent.riddles.masters
        : undefined,
    ),
  ...
  const clues = useMemo(
    () => mergeRiddleClues(
      wikiArticles,
      hiddenIds,
      siteSettings.parsedContent?.riddles?.clues?.length
        ? siteSettings.parsedContent.riddles.clues
        : undefined,
    ),
  ...
  const handleDelete = useCallback((item: RiddleMaster | RiddleClue) => {
    const label = 'clueEn' in item ? (item.clueRu || item.clueEn) : item.nameRu;
  ...
  const filteredClues = useMemo(() => {
    ...
    return clues.filter(c => {
      if (!q) return true;
      return `${c.clueRu || ''} ${c.clueEn} ${c.answers.join(' ')}`.toLowerCase().includes(q);
    });
  }, [clues, search]);
...
function CompactClueCard({
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <p className="text-xs text-white capitalize truncate leading-tight">{clue.clueRu || clue.clueEn}</p>
        <p className="text-[10px] text-gold-300/80 truncate">{clue.primaryAnswer}</p>
        {clue.clueRu && clue.clueEn && clue.clueRu !== clue.clueEn && (
          <p className="text-[9px] text-ink-600 truncate">{clue.clueEn}</p>
        )}

...
export default function SupportWidget({ onLoginClick }: SupportWidgetProps) {
  const {
    ...
    hasPermission, getUserDisplayName,
  } = useAuth();
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {!user ? (
            ...
            <div className="space-y-3">
              ...
              <div className="bg-ink-800/50 border border-ink-700/30 rounded-lg p-2">
              ...
              {(canClose || canDelete || activeTicket.userId === user.id) && activeTicket.status !== 'closed' && (
                <div className="flex gap-2 pt-1">
                  {(canClose || activeTicket.userId === user.id) && (
                    <button onClick={() => { closeTicket(activeTicket.id); setView('list'); setActiveTicketId(null); }}
              ...
              {confirmDelete && (
                <div className="bg-crimson-400/10 border border-crimson-400/30 rounded-lg p-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { deleteTicket(activeTicket.id); setConfirmDelete(false); setView('list'); setActiveTicketId(null); }}

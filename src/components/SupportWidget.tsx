...
export default function SupportWidget({ onLoginClick }: SupportWidgetProps) {
  ...
  // Track unread/new tickets and replies since last widget view
  useEffect(() => {
    if (isOpen && !isMinimized) {
      setUnreadCount(0);
      setLastSeenCount(myTickets.length);
    } else {
      const newItems = myTickets.length - lastSeenCount;
      if (newItems > 0) setUnreadCount(newItems);
    }
  }, [myTickets.length, isOpen, isMinimized]);
  ...
  // Keep opened ticket synced with incoming updates from server
  useEffect(() => {
    if (!selectedTicket) return;
    const fresh = supportTickets.find(t => t.id === selectedTicket.id);
    if (fresh) {
      const prevStr = JSON.stringify(selectedTicket);
      const nextStr = JSON.stringify(fresh);
      if (prevStr !== nextStr) setSelectedTicket(fresh);
    }
  }, [supportTickets, selectedTicket?.id]);
  ...
  if (!isOpen) {
    return (
      <button onClick={() => { setIsOpen(true); setIsMinimized(false); setUnreadCount(0); setLastSeenCount(myTickets.length); }}
      ...
      <div className="bg-ink-900 border border-purple-500/30 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden flex flex-col"
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-purple-950/60 border-b border-purple-500/20 shrink-0 cursor-pointer"
          ...
          <div className="flex items-center gap-2">
            ...
            {unreadCount > 0 && <span className="text-[10px] bg-gold-400/20 text-gold-400 px-1.5 py-0.5 rounded-full">{unreadCount}</span>}
          </div>

      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-ink-300 max-w-xl mx-auto">
            Ответы на самые популярные вопросы о Where Winds Meet
          </p>
          ...
          {faqs.map((faq, index) => (
            <div
              ...
              className={`bg-ink-800/50 border rounded-xl overflow-hidden transition-all duration-300 ${
                openIndex === index ? 'border-gold-400/30' : 'border-ink-700/30'
              }`}
            >
              <button
              ...
              >
                <HelpCircle className={`w-5 h-5 mt-0.5 shrink-0 transition-colors ${
                  openIndex === index ? 'text-gold-400' : 'text-ink-400 group-hover:text-gold-400'
                }`} />
                <span className={`font-serif font-bold flex-1 transition-colors ${
                  openIndex === index ? 'text-gold-400' : 'text-white group-hover:text-gold-300'
                }`}>
                  {faq.question}
                </span>
              ...
              {openIndex === index && (
                <div className="px-5 pb-5 animate-fadeIn">
                  <div className="pl-8 border-l-2 border-gold-400/20">
                    <p className="text-ink-200 leading-relaxed">{faq.answer}</p>
                  </div>
                </div>
              )}
            </div>

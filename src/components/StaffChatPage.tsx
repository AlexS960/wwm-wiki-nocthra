...
import { ArrowLeft, MessageSquare, Search, ShieldAlert, Users, UsersRound, Palette, Trash2, MoreVertical, Pencil, Check, X } from 'lucide-react';
...
import StaffChatRoleFilter from './staffChat/StaffChatRoleFilter';
...
export default function StaffChatPage({ onBack, onLoginClick }: StaffChatPageProps) {
  ...
  const loadStaff = useCallback(async () => {
    ...
    try {
      ...
      const views = accs.map(a => accountToStaffView(a, getRoleConfig));
      setStaffList(views);
    } finally {
  ...
  const handleSend = async () => {
    ...
    if (activeParsed?.type === 'dm') {
      ...
      if (!partner || !isStaffChatRole(partner.role, siteSettings.roles)) {
  ...
  const openDialogMenu = (e: React.MouseEvent, chatKey: string) => {
    e.preventDefault();
    e.stopPropagation();
    setDialogMenu({ chatKey, x: e.clientX, y: e.clientY });
  };

  const handleDeleteDialogForMe = async (chatKey: string) => {
    setDialogMenu(null);
    if (!user) return;
  ...
  const handleDeleteDialogForAll = async (chatKey: string) => {
    setDialogMenu(null);
    const parsed = parseStaffChatKey(chatKey);
    if (!parsed) return;
    ...
    if (parsed.type === 'dm') {
      ...
      const isAdminUser = user?.role === 'admin' || getRoleConfig(user?.role || '').permissions?.includes('admin.panel');
    ...
    <div className="h-[calc(100dvh-4rem)] md:h-[calc(100dvh-5rem)] pt-16 md:pt-20 flex flex-col bg-ink-900/40">
              ...
              filteredConversations.map(chat => {
                ...
                return (
                  <button
                    ...
                    onContextMenu={e => openDialogMenu(e, chat.key)}
            ...
            <>
              <div className="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-ink-700/40 bg-ink-800/50 relative">
                ...
                <div className="min-w-0 flex-1 flex items-center gap-2">
                  {activeParsed?.type === 'group' && editingGroupTitle ? (
                    <>
                      <input
                      ...
                      />
                      <button type="button" onClick={() => void saveGroupTitle()} className="p-1 text-jade-400 cursor-pointer"><Check className="w-4 h-4" /></button>
                      <button type="button" onClick={() => setEditingGroupTitle(false)} className="p-1 text-ink-400 cursor-pointer"><X className="w-4 h-4" /></button>
                    </>
                  ) : (
                    <>
                      <div className="min-w-0 flex-1">
                      ...
                      {activeParsed?.type === 'group' && (
                        <button
                          type="button"
                          onClick={() => { setGroupTitleDraft(activeConv?.title || ''); setEditingGroupTitle(true); }}
                          className="p-1.5 text-ink-400 hover:text-purple-400 cursor-pointer shrink-0"
                ...
                <button
                  type="button"
                  onClick={(e) => activeChatKey && openDialogMenu(e, activeChatKey)}
                  className="p-2 rounded-lg text-ink-400 hover:text-ink-200 cursor-pointer shrink-0"
      ...
      {dialogMenu && (
        <div
          ...
          style={{ left: Math.min(dialogMenu.x, window.innerWidth - 240), top: Math.max(8, dialogMenu.y - 8) }}
          onClick={e => e.stopPropagation()}
        >
          <button
            type="button"
            onClick={() => void handleDeleteDialogForMe(dialogMenu.chatKey)}
          ...
          <button
            type="button"
            onClick={() => void handleDeleteDialogForAll(dialogMenu.chatKey)}

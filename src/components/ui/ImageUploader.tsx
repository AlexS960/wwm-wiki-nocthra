...
import { isEmbeddedImageUrl } from '../../lib/siteImages';
...
export default function ImageUploader({ images, onChange, storageFolder = 'uploads' }: ImageUploaderProps) {
  ...
  const hasLegacyEmbedded = images.some(isEmbeddedImageUrl);
  ...
  return (
    <div>
      ...
      {hasLegacyEmbedded && (
        <p className="text-orange-400/90 text-[10px] mb-2">
          Есть старые встроенные картинки (base64). Удалите их и загрузите заново через «Добавить», либо нажмите «Очистить base64» в админке.
        </p>
      )}
      <div className="flex flex-wrap gap-2">
        {images.map((src, i) => (
          <div key={src + i} className={`relative w-20 h-20 sm:w-24 sm:h-24 rounded-lg overflow-hidden border group ${isEmbeddedImageUrl(src) ? 'border-orange-500/50' : 'border-ink-600/50'}`}>

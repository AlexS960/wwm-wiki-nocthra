...
import type { CustomSectionDefinition, SectionCategoryDef, SectionFieldDef } from '../../types/site';
import { useSectionCategories } from '../../hooks/useSectionCategories';
import { mergeCategoriesDefinitionFirst } from '../../lib/sectionCategoriesMerge';
...
function buildCategoryOptions(
  definition: CustomSectionDefinition,
  fromHook: SectionCategoryDef[],
): SectionCategoryDef[] {
  const fromDef = definition.categories || [];
  const merged = fromDef.length
    ? mergeCategoriesDefinitionFirst(fromDef, fromHook)
    : fromHook;
  return merged.length ? merged : [{ id: 'прочее', label: 'Прочее', icon: '✦' }];
}

function buildInitialValues(
  categoryOptions: SectionCategoryDef[],
  iconChoices: string[],
  initial?: Partial<DynamicEditorValues>,
): DynamicEditorValues {
  const defaultCat = categoryOptions[0]?.id || '';
  const fields = { ...(initial?.fields || {}) };
  if (!fields.category && defaultCat) fields.category = defaultCat;
  return {
    title: initial?.title || '',
...
function fieldEffectiveValue(
  values: DynamicEditorValues,
  field: SectionFieldDef,
  categoryOptions: SectionCategoryDef[],
): string {
  if (field.key === 'title') return values.title;
  if (field.key === 'icon') return values.icon;
  if (field.key === 'content') return values.fields.content || '';
  if (field.key === 'category') return values.fields.category || categoryOptions[0]?.id || '';
  return values.fields[field.key] || '';
}
...
export default function DynamicSectionEditorModal({
  ...
  const categoryOptions = useMemo(
    () => buildCategoryOptions(definition, categories),
  ...
  const [values, setValues] = useState<DynamicEditorValues>(() =>
    buildInitialValues(categoryOptions, iconChoices, initial),
  );

  useEffect(() => {
    setValues(buildInitialValues(categoryOptions, iconChoices, initial));
  }, [initial, iconChoices, categoryOptions]);

  const requiredOk = useMemo(() => {
    return definition.fields.every(f => {
      if (!f.required) return true;
      const v = fieldEffectiveValue(values, f, categoryOptions);
      return Boolean(String(v).trim());
    });
  }, [definition.fields, values, categoryOptions]);
  ...
  const renderField = (field: SectionFieldDef) => {
    ...
    switch (field.type) {
      ...
      case 'category':
        return (
          <select
            value={fieldEffectiveValue(values, field, categoryOptions)}
            onChange={e => updateField(field, e.target.value)}
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-ink-700/50 bg-ink-900/50">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!fields.category && categoryOptions[0]?.id) {
                fields.category = categoryOptions[0].id;
              }

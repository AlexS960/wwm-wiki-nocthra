...
export default function ContentFormatToolbar({ value, onChange, textareaRef }: ContentFormatToolbarProps) {
  ...
  const apply = (fn: (v: string, start: number, end: number) => { value: string; cursor: number }) => {
  ...
  return (
    <div className="border-b border-gold-500/20 bg-ink-900/70">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-ink-700/40">
        <Type className="w-3.5 h-3.5 text-gold-400 shrink-0" />
        <span className="text-xs font-medium text-gold-400/90">Форматирование текста</span>
      </div>

...
export default function FloatingChat({ onLoginClick }: FloatingChatProps) {
  ...
  const visibleMessages = useMemo(
    () => (chatSearchResults ?? chatState.messages).filter(m => !m.deleted),
    [chatSearchResults, chatState.messages],
  );
  ...
  }, [chatSearchResults, chatHasMore, chatLoadingMore, loadOlderChatMessages]);
  ...
  const getRoleDisplay = (roleId: string) =>
    (siteSettings.roles ?? []).find(r => r.id === roleId) || { displayName: roleId, color: '#b0a696' };
        ...
        {!isMinimized && (
          <>
            ...
            <div
              ...
              className="scroll-area flex-1 min-h-0 px-3 py-2 space-y-0.5"

...
export default function FloatingChat({ onLoginClick }: FloatingChatProps) {
  const {
    ...
    isUserMuted, chatBanUser, hasPermission, siteSettings
  } = useAuth();
  ...
  const isMod = hasPermission('chat.delete');
  ...
  useEffect(() => {
    if (isOpen && !isMinimized) {
      ...
      const newMessages = visibleMessages.length - lastSeenCount;
    ...
    <div className={`fixed left-4 sm:left-6 bottom-4 sm:bottom-6 z-50 transition-all duration-300 ${
      ...
      <div className="bg-ink-900 border border-gold-700/30 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden flex flex-col"
        ...
        {!isMinimized && (
          <>
            {/* Messages */}
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-0.5" onClick={() => setActionMenu(null)}>
              {!user ? (
                  ...
                  return (
                    <div key={msg.id}
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="font-semibold text-xs" style={{ color: roleDisplay.color }}>{msg.userName}</span>
                      ...
                      {/* Message text */}
                      <p className="text-ink-200 text-[13px] leading-snug break-words mt-0.5">{msg.text}</p>
                    </div>
                  );
                })
              )}
            ...
            {/* Input */}
            {user && (
              <div className="px-3 py-2.5 bg-ink-800/50 border-t border-ink-700/30 shrink-0">
                {currentMute ? (
                  <div className="flex items-center justify-center gap-2 py-2 text-orange-400 text-xs">
                    <AlertTriangle className="w-4 h-4" />
                    <span>Мут: {formatMuteRemaining(currentMute.until)}</span>

...
export default function HeroSection({ onNavigate: _onNavigate, hasAnnouncements = false }: HeroSectionProps) {
  const { hasPermission, discordUrl, updateDiscordUrl } = useAuth();
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className={`relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center ${hasAnnouncements ? 'pt-6 md:pt-8' : 'pt-16 md:pt-20'}`}>
        <div className="animate-fadeInUp">
          <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl font-bold text-shadow-glow mb-4 flex flex-wrap items-center justify-center gap-3 sm:gap-4">
            <img
              src={WWM_LOGO}
              alt=""
              className="h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 object-contain drop-shadow-[0_0_12px_rgba(212,175,55,0.35)] shrink-0"
              onError={e => {

...
export default function HeroSection({ onNavigate, hasAnnouncements = false }: HeroSectionProps) {
  ...
  const hero = mergeHeroSettings(siteSettings.hero);
  const lolkaUrl = getResolvedLolkaUrl(siteSettings);
  const logoUrl = hero.logoUrl?.trim() || DEFAULT_LOGO;
  ...
  const handleLolkaSave = () => {
    ...
    if (!url) return;
    updateSiteSettings({ lolkaUrl: url });
    setLolkaSaved(true);
        ...
        <div className="md:animate-fadeInUp">
          ...
          <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl font-bold text-shadow-glow mb-4 flex flex-wrap sm:flex-nowrap items-center justify-center gap-0.5 sm:gap-1 md:gap-2">
            ...
            <span>
              ...
              <span className="text-gold-300">{hero.titleGold}</span>
            </span>
          </h1>
        ...
        <div className="mb-10 md:animate-fadeInUp relative inline-flex flex-wrap justify-center gap-3" style={{ animationDelay: '0.5s' }}>
          <a href={discordUrl} target="_blank" rel="noopener noreferrer"
            ...
            <div className="text-left">
              <div className="text-white font-semibold text-sm group-hover:text-white transition-colors drop-shadow-[0_0_6px_rgba(255,255,255,0.25)]">{hero.discordTitle}</div>
              <div className="text-[#d8e0ff] text-xs group-hover:text-white transition-colors">{hero.discordSubtitle}</div>
            </div>
          </a>
          <a href={lolkaUrl} target="_blank" rel="noopener noreferrer"
            ...
            <div className="text-left">
              <div className="text-white font-semibold text-sm group-hover:text-white transition-colors drop-shadow-[0_0_6px_rgba(255,255,255,0.25)]">{hero.lolkaTitle}</div>
              <div className="text-[#f0dcff] text-xs group-hover:text-white transition-colors">{hero.lolkaSubtitle}</div>
            </div>
          </a>

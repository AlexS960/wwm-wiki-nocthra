...
import { dbLoadSiteData, dbSaveSiteData } from '../lib/db';
...
export default function HeroSection({ onNavigate: _onNavigate }: HeroSectionProps) {
  ...
  useEffect(() => {
    dbLoadSiteData<string>('discord_url', 'https://discord.com/invite/mYqKkN3u4').then(url => {
  ...
  const handleSave = () => {
    ...
    if (!url) return;
    setDiscordUrl(url);
    dbSaveSiteData('discord_url', url);
    setSaved(true);
        ...
        <div className="mb-10 animate-fadeInUp relative inline-block" style={{ animationDelay: '0.5s' }}>
          ...
          {showEditor && (
            <>
              ...
              <div
                className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[min(20rem,calc(100vw-2rem))] bg-ink-900/98 backdrop-blur-md border border-[#5865F2]/30 
                           rounded-xl shadow-2xl p-4 z-[70] animate-fadeIn"
                onClick={e => e.stopPropagation()}
              >
                <div className="flex items-center justify-between mb-3">
                  <span className="text-white text-sm font-medium">Ссылка Discord</span>
                  <button onClick={() => setShowEditor(false)} className="text-ink-400 hover:text-white cursor-pointer">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-ink-800 border border-ink-600/40 rounded-lg px-3 py-2 text-sm text-white 
                             placeholder:text-ink-500 focus:outline-none focus:border-[#5865F2]/50"
                    autoFocus
                    onKeyDown={e => {
                ...
                <p className="text-ink-500 text-[10px] mt-2">
                  Вставьте бессрочную ссылку-приглашение Discord
                </p>
              </div>
            </>
          )}

  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_30%,rgba(15,11,7,0.7)_100%)]" />
      </div>

      <div className={`relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center ${hasAnnouncements ? 'pt-6 md:pt-8' : 'pt-16 md:pt-20'}`}>
        <div className="animate-fadeInUp">
          <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl font-bold text-shadow-glow mb-4 flex flex-wrap sm:flex-nowrap items-center justify-center gap-0.5 sm:gap-1md:gap-2">
            <img
              src={WWM_LOGO}
              alt=""
        ...
        <div className="mb-10 animate-fadeInUp relative inline-flex flex-wrap justify-center gap-3" style={{ animationDelay: '0.5s' }}>
          ...
          <a href={lolkaUrl} target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-3 px-6 py-3 rounded-xl
                       bg-[#a855f7]/20 border border-[#a855f7]/40 backdrop-blur-sm
                       hover:bg-[#a855f7]/30 hover:border-[#a855f7]/60 hover:scale-105
                       transition-all duration-300 group">
            <span className="w-5 h-5 rounded-full bg-[#a855f7]/30 text-[#e9d5ff] flex items-center justify-center text-[10px] font-bold">L</span>
            <div className="text-left">
              <div className="text-white font-semibold text-sm group-hover:text-white transition-colors">Lolka</div>
              <div className="text-[#a855f7]/70 text-xs group-hover:text-[#d8b4fe] transition-colors">Перейти на Lolka</div>
            </div>
          </a>
          ...
          {canEdit && (
            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setLolkaDraft(lolkaUrl); setShowLolkaEditor(true); setLolkaSaved(false); }}
              className="absolute -top-2 -left-2 w-7 h-7 rounded-full bg-ink-800 border border-[#a855f7]/40 

...
export default function SectsSection() {
  ...
  const items = useMemo(() => (
    Array.isArray(sectionOverrides.sects) ? (sectionOverrides.sects as typeof sects) : sects
  ), [sectionOverrides]);
  const persistItems = (next: typeof sects) => {
    updateSiteSettings({ sectionOverrides: { ...sectionOverrides, sects: next } });
  };
          ...
          {items.map(sect => (
            <div key={sect.id} onClick={() => setExpandedSect(expandedSect === sect.id ? null : sect.id)}
              ...
              <div className="flex items-start gap-3 mb-3">
                ...
                {expandedSect === sect.id ? <ChevronUp className={`w-5 h-5 text-gold-400 ${canManage ? 'mr-14' : ''}`} /> : <ChevronDown className={`w-5 h-5 text-ink-400 ${canManage ? 'mr-14' : ''}`} />}
              </div>
              {canManage && (
                <div className="absolute top-3 right-3 flex items-center gap-1 z-10" onClick={e => e.stopPropagation()}>
                  ...
                  <button
                    onClick={() => {
                      ...
                      persistItems(items.filter(x => x.id !== sect.id));
                    }}

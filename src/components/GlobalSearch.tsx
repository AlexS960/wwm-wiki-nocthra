...
export default function GlobalSearch({ onNavigate }: GlobalSearchProps) {
  ...
  const [remoteGuides, setRemoteGuides] = useState<typeof guides>([]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      void searchGuidesRemote(q).then(setRemoteGuides);
    }, 250);
    return () => clearTimeout(t);
  }, [query, searchGuidesRemote]);

  const guideSource = query.trim().length >= 2 ? remoteGuides : guides;

  const sectionLabels = useMemo(() => getWikiSectionLabels(siteSettings), [siteSettings]);
  ...
  useEffect(() => {
    ...
    setSearching(true);
    const t = setTimeout(() => {
      void searchSite(q, guideSource, wikiArticles, npcs, 20, sectionLabels).then(r => {
        if (active) {
          setResults(r);
          setSearching(false);
        }
  ...
  }, [query, guideSource, wikiArticles, npcs, sectionLabels]);

  useEffect(() => {
    if (open) {
      preloadSearchExtras();
      void Promise.all([ensureGuidesLoaded(), ensureWikiLoaded(), loadBuiltinNpcs().then(setNpcs)]);
    ...
    <div ref={wrapRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-[min(100vw-2rem,30rem)] bg-ink-900/98 backdrop-blur-md border border-gold-700/35 rounded-xl shadow-2xl z-[60] overflow-hidden animate-fadeIn">
          ...
          <div className="scroll-area max-h-72 py-1">
            {query.trim().length < 2 ? (
              <p className="px-4 py-6 text-ink-500 text-xs text-center">Введите минимум 2 символа</p>
            ) : searching ? (
              <p className="px-4 py-6 text-ink-500 text-xs text-center">Поиск…</p>
            ) : results.length === 0 ? (

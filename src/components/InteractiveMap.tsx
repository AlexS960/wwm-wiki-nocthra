...
const mapSources = [
  { id: 'mapgenie', name: 'MapGenie', desc: 'Все коллекционные, прогресс, фильтры', url: 'https://mapgenie.io/where-winds-meet/maps/world', embedUrl: 'https://mapgenie.io/where-winds-meet/maps/world?embed=true', icon: '🗺️' },
  { id: 'bgone', name: 'BgoneGaming', desc: '35+ типов, оффлайн трекинг', url: 'https://bgonegaming.win/wherewindmeets/', embedUrl: 'https://bgonegaming.win/wherewindmeets/', icon: '🌐' },
  { id: 'community', name: 'Community Map', desc: '8000+ локаций, скриншоты', url: 'https://earth-revival-interactive-map.pages.dev/wherewindmeets/', embedUrl: 'https://earth-revival-interactive-map.pages.dev/wherewindmeets/', icon: '👥' },
];
    ...
    <section id="map" className="py-20 bg-ink-900/80">
      <div className={isFullscreen ? 'fixed inset-0 z-50 bg-ink-900 p-0' : 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'}>
        ...
        <div className={`flex ${isFullscreen ? 'h-screen' : 'h-[75vh] rounded-xl overflow-hidden border border-gold-700/30'}`}>
          <div className={`bg-ink-900 border-r border-ink-700/50 flex flex-col transition-all duration-300 shrink-0 ${
            ...
            <div className="p-3 border-b border-ink-700/50 shrink-0">
              <h3 className="font-serif text-sm font-bold text-gold-400 flex items-center gap-2"><Map className="w-4 h-4" /> Категории карты</h3>
              <p className="text-ink-500 text-[10px] mt-1">Используйте фильтры на встроенной карте</p>
            </div>
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              {categories.map(cat => (
                <div key={cat.group}>
                  <button onClick={() => toggleGroup(cat.group)}
                  ...
                  {expandedGroups.has(cat.group) && (
                    <div className="space-y-0.5 mb-2">
                      {cat.items.map(item => (
                        <div key={item.nameEn} className="flex items-center gap-2 px-3 py-1.5 rounded-md text-xs text-ink-200 hover:bg-ink-700/40 transition-colors group">
          ...
          <div className="flex-1 flex flex-col bg-ink-800 min-w-0">
            ...
            <div className="flex-1 relative">
              <iframe key={activeSource.id} src={activeSource.embedUrl}
        ...
        <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          <WikiArticleCards sectionId="map" />
        </div>
      </div>
    </section>
  );

...
export default function BuildsSection() {
  ...
  const { user, progress, setSelectedBuild, isEditor, isAdmin, siteSettings, updateSiteSettings } = useAuth();
  const canManage = isEditor() || isAdmin();
  const sectionOverrides = siteSettings.sectionOverrides || {};
  const items = useMemo(() => (
    Array.isArray(sectionOverrides.builds) ? (sectionOverrides.builds as typeof buildPaths) : buildPaths
  ), [sectionOverrides]);
  const persistItems = (next: typeof buildPaths) => {
    updateSiteSettings({ sectionOverrides: { ...sectionOverrides, builds: next } });
  };
      ...
      <div className="relative">
        <button onClick={onToggle} className="w-full text-left p-5 cursor-pointer">
          <div className="flex items-start justify-between gap-2">
            ...
            {isExpanded ? <ChevronUp className={`w-5 h-5 text-ink-400 ${canManage ? 'mr-14' : ''}`} /> : <ChevronDown className={`w-5 h-5 text-ink-400 ${canManage ? 'mr-14' : ''}`} />}
          </div>

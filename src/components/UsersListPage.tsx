...
export default function UsersListPage({ onBack }: UsersListPageProps) {
  const { registeredUsers, isUserOnline, getRoleConfig, siteSettings } = useAuth();
  ...
  // Filter out admin account and demo system accounts
  const visibleUsers = registeredUsers.filter(u => u.role !== 'admin');

...
export default function WeaponsSection() {
  ...
  const { progress, toggleFavoriteWeapon, isEditor, isAdmin, siteSettings, updateSiteSettings } = useAuth();
  const canManage = isEditor() || isAdmin();
  const sectionOverrides = siteSettings.sectionOverrides || {};
  const items = useMemo(() => (
    Array.isArray(sectionOverrides.weapons) ? (sectionOverrides.weapons as typeof weapons) : weapons
  ), [sectionOverrides]);
  const persistItems = (next: typeof weapons) => {
    updateSiteSettings({ sectionOverrides: { ...sectionOverrides, weapons: next } });
  };
    ...
    <div onClick={() => setExpanded(!expanded)}
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <button onClick={onToggleFavorite}
          className={`absolute top-3 left-3 p-1.5 rounded-lg transition-colors cursor-pointer ${isFavorite ? 'text-gold-400' : 'text-ink-600 hover:text-gold-400'}`}>

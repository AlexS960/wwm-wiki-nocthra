...
function formatAiLine(ai: SyncResult['ai']): string {
  if (!ai) return '';
  if (ai.error) return ` · DeepSeek: ${ai.error}`;
  if (ai.used) return ` · DeepSeek: ${ai.message || `+${ai.enriched ?? 0}`}`;
  if (ai.reason === 'no_key_or_payload') return '';
...
export default function ParsersPanel() {
  ...
  const [aiStatus, setAiStatus] = useState<DeepSeekStatus | null>(null);
  ...
  const refreshAiStatus = useCallback(async () => {
    ...
    try {
      ...
      setAiStatus({ configured: false, active: false, message: 'Не удалось проверить DeepSeek' });
    } finally {
  ...
  const runSection = useCallback(async (sectionId: string) => {
    ...
    if (useAi && aiStatus?.configured) {
      pushLog({ section: sectionId, status: 'ai', message: 'Парсинг + DeepSeek…' });
    }
  ...
  const handleRun = useCallback(async (sectionId: string) => {
    ...
    try {
      ...
      if (result.error || result.skipped) {
        ...
        if (applyToSite && result.payload && !isPayloadEmpty(result.payload)) {
          applyResult(result);
        }
        const diffStr = formatSyncDiff(result.diff);
        let msg = `Записей: ${result.count ?? 0}${diffStr ? ` (${diffStr})` : ''}`;
        ...
        if (result.discovered?.matched) msg += ' · авто-ссылка';
        msg += formatAiLine(result.ai);
        if (applyToSite && result.payload && !isPayloadEmpty(result.payload)) msg += ' · применено на сайте';
        else if (applyToSite && !dryRun && !result.payload) msg += ' · без данных для Supabase';
        pushLog({
          section: sectionId,
          status: result.ai?.used ? 'ai' : 'ok',
          message: msg,
        });
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-ink-800/40 border border-ink-700/50 rounded-xl p-5">
        ...
        <p className="text-ink-400 text-sm mb-4">
          ...
          Или укажите ссылки вручную. DeepSeek переводит и дополняет записи после парсинга.
        </p>

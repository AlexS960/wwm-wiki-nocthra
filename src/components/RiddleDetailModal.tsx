import { X, MapPin, Lightbulb, Coins, Brain, Edit3, Trash2 } from 'lucide-react';
...
export default function RiddleDetailModal(props: Props) {
  const { onClose, canEdit, onEdit, onDelete } = props;
      ...
      <div className="bg-ink-800 border border-gold-700/30 rounded-2xl max-h-[88vh] overflow-y-auto relative">
        <div className="absolute top-3 right-3 flex items-center gap-1 z-10">
          {canEdit && onEdit && onDelete && (
            <>
              <button
                type="button"
                onClick={onEdit}
                className="p-2 rounded-lg text-gold-400 hover:bg-gold-400/10 cursor-pointer"
              ...
              <button
                type="button"
                onClick={onDelete}
                className="p-2 rounded-lg text-ink-500 hover:text-crimson-400 hover:bg-crimson-400/10 cursor-pointer"
                aria-label="Удалить"
              >
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-lg text-ink-400 hover:text-white hover:bg-ink-700/60 cursor-pointer"
            aria-label="Закрыть"
          >
            <X className="w-5 h-5" />
          </button>

      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-3 mb-12">
          {filteredTips.map(tip => (
            <div key={tip.id} className="bg-ink-800/50 border border-ink-700/30 rounded-xl p-4 flex items-start gap-3">
              ...
              {canManage && (
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => setEditId(tip.id)} className="px-2 py-1 rounded-lg text-[10px] text-gold-300 border border-gold-400/30 hover:bg-gold-400/10 cursor-pointer inline-flex items-center gap-1">
                    <Edit3 className="w-3 h-3" /> Редактировать
                  </button>
                  <button
                    onClick={() => {
                      if (!confirm('Удалить совет?')) return;
                      setTipItems(prev => prev.filter(x => x.id !== tip.id));
                    }}
                    className="px-2 py-1 rounded-lg text-[10px] text-crimson-300 border border-crimson-400/30 hover:bg-crimson-400/10 cursor-pointer inline-flex items-center gap-1"
                  >

...
export default function CookingSection() {
  ...
  const items = useMemo(() => (
    Array.isArray(sectionOverrides.cooking) ? (sectionOverrides.cooking as typeof recipes) : recipes
  ), [sectionOverrides]);
  const persistItems = (next: typeof recipes) => {
    updateSiteSettings({ sectionOverrides: { ...sectionOverrides, cooking: next } });
  };

...
export default function PrivateMessages({ onLoginClick }: PrivateMessagesProps) {
  const {
    ...
    deletePrivateMessageForMe, deletePrivateMessageForAll,
  ...
  }, [isOpen, view, activeChatId, markPMRead, scrollChatToBottom]);
            ...
            {!user ? (
              ...
              !pmLoaded ? (
                ...
                <div className="space-y-1">
                  {chats.map(chat => (
                    <button key={chat.id} onClick={() => { setActiveChatId(chat.id); setView('chat'); stickToBottomRef.current = true; }}

...
import { Mail, X, Send, User, ArrowLeft, Smile, Settings, Volume2, VolumeX } from 'lucide-react';
...
export default function PrivateMessages({ onLoginClick }: PrivateMessagesProps) {
  const {
    ...
    siteSettings, hasPermission, getUserDisplayName, dbSaveError, clearDbSaveError,
  } = useAuth();
  ...
  useEffect(() => {
    if (isOpen && view === 'chat' && activeChatId) markPMRead(activeChatId);
  }, [isOpen, view, activeChatId, privateMessages]);

  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [privateMessages, view, activeChatId]);

  // Звук при новом входящем ЛС
  useEffect(() => {
  ...
  const activeChatName = activeChatId ? getUserDisplayName(activeChatId, chats.find(c => c.id === activeChatId)?.name || '') : '';
  ...
  const handleCompose = async () => {
    ...
    setSending(false);
    if (err) setSendError(err);
    else { setText(''); setActiveChatId(recipientId); setView('chat'); }
  };
  ...
  return (
    <div className="fixed right-2 sm:right-6 bottom-20 sm:bottom-6 z-50 w-[min(100vw-1rem,380px)] max-w-[calc(100vw-1rem)]">
      <div className="bg-ink-900 border border-purple-700/30 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden flex flex-col" style={{ height: 'min(500px, calc(100dvh - 6rem))' }}>
        <div className="flex items-center justify-between px-4 py-2.5 bg-ink-800/80 border-b border-ink-700/30 shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            {view !== 'list' && view !== 'settings' && (
              <button onClick={() => { setView('list'); setActiveChatId(null); setShowEmoji(false); }} className="p-1 text-ink-400 hover:text-white cursor-pointer shrink-0"><ArrowLeft className="w-4 h-4" /></button>
            )}
            {view === 'settings' && (
        ...
        {displayError && (
          <div className="mx-3 mt-2 px-3 py-2 rounded-lg bg-crimson-400/10 border border-crimson-400/30 text-crimson-300 text-xs flex justify-between gap-2">
        ...
        <div className="flex-1 overflow-y-auto p-3 min-h-0">
          {!user ? (
            ...
            <div className="space-y-4">
              ...
              {isAdmin && (
                <PmAdminSoundSettings />
              )}
              <p className="text-ink-500 text-[10px]">Сообщения до 1000 символов. Отображается игровой ник собеседника.</p>
            </div>
          ) : view === 'list' ? (
            chats.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-8">

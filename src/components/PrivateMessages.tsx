...
export default function PrivateMessages({ onLoginClick }: PrivateMessagesProps) {
  const {
    ...
    siteSettings, hasPermission, getUserDisplayName,
  } = useAuth();
  ...
  const handleSend = () => {
    ...
    sendPrivateMessage(activeChatId, text.trim());
    setText('');
    setShowEmoji(false);
  };
  ...
  const handleCompose = () => {
    ...
    sendPrivateMessage(recipientId, text.trim());
    setText(''); setRecipientId(''); setActiveChatId(recipientId); setView('chat');
  };

...
import { Mail, X, Send, User, ArrowLeft, Smile, Settings, Volume2, VolumeX, MoreVertical, Trash2, Type } from 'lucide-react';
...
export default function PrivateMessages({ onLoginClick }: PrivateMessagesProps) {
  ...
  const applyBbCode = (openTag: string, closeTag: string, placeholder = 'текст') => {
    const target = view === 'compose' ? composeInputRef.current : chatInputRef.current;
    if (!target) return;
            ...
            {user && (
              <div className="px-3 py-2.5 bg-ink-800/45 border-t border-ink-700/35 shrink-0 relative">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input ref={chatInputRef} value={text} onChange={e => setText(e.target.value)} placeholder="Сообщение... (BBCode: [b], [i], [url])" maxLength={1000}
          ...
          <div className="flex-1 overflow-y-auto p-3 min-h-0">
            {!user ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <p className="text-ink-400 text-xs">BBCode доступен</p>
                  <button
                    type="button"
                ...
                <textarea ref={composeInputRef} value={text} onChange={e => setText(e.target.value)} rows={4} placeholder="Сообщение... (поддержка BBCode)" maxLength={1000}

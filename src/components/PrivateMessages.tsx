            ...
            {!user ? (
              ...
              !pmLoaded ? (
                ...
                <div className="space-y-1">
                  {chats.map(chat => (
                    <div key={chat.id} className="w-full text-left bg-ink-800/50 border border-ink-700/30 rounded-xl p-3 hover:border-purple-500/20 flex items-center gap-3">
                      <button
                        type="button"
                      ...
                      <div className="flex flex-col gap-1 shrink-0">
                        <button
                          type="button"
                          onClick={async () => {
                            const err = await deletePmDialogForMe(chat.id);
                            if (err) setSendError(err);
                          }}
                          className="px-2 py-1 text-[10px] rounded bg-ink-700/60 text-ink-300 hover:bg-ink-700 cursor-pointer"
                        >
                          Удалить у меня
                        </button>
                        <button
                          type="button"
                          onClick={async () => {
                            const err = await deletePmDialogForAll(chat.id);
                            if (err) setSendError(err);
                          }}
                          className="px-2 py-1 text-[10px] rounded bg-crimson-500/20 text-crimson-300 hover:bg-crimson-500/30 cursor-pointer"
                        >
                          Удалить у всех
                        </button>
                      </div>

...
export default function ProfileModal({ isOpen, onClose }: ProfileModalProps) {
  const { user, progress, guides, logout, deleteNote, addNote, setSelectedBuild, getRoleConfig, updateUserPicture, updateUserGameNickname, isUserOnline } = useAuth();
      ...
      <div className="relative w-full max-w-5xl flex flex-col sm:flex-row items-start gap-5">
        ...
        <div className="shrink-0 rounded-2xl border border-gold-400/25 bg-ink-900/70 backdrop-blur-sm p-3 shadow-2xl shadow-black/25">
          ...
          <div className="mt-3 flex items-center justify-center gap-2 text-xs">
            <span className={`w-2.5 h-2.5 rounded-full ${isUserOnline(user.id) ? 'bg-jade-400 shadow-[0_0_10px_rgba(74,191,133,0.7)]' : 'bg-ink-500'}`} />
            <span className={isUserOnline(user.id) ? 'text-jade-400' : 'text-ink-500'}>
              {isUserOnline(user.id) ? 'Онлайн' : 'Оффлайн'}
            </span>
          </div>
        </div>

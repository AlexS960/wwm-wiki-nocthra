...
export default function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const handleLogin = () => {
    ...
    const err = loginWithPassword(username.trim(), password, remember);
        ...
        <div className="p-6 space-y-4" onKeyDown={handleKeyDown}>
          ...
          {mode === 'login' && (
            <label className="flex items-center gap-2.5 cursor-pointer group">
              ...
              <span className="text-ink-300 text-sm select-none" onClick={() => setRemember(!remember)}>Запомнить меня</span>
            </label>
          )}
          ...
          <button
            ...
            disabled={!username.trim() || !password || (mode === 'register' && !passwordConfirm)}
          ...
          <p className="text-center text-ink-500 text-xs">
            {mode === 'login' ? (
              <>Нет аккаунта? <button onClick={() => switchMode('register')} className="text-gold-400 hover:text-gold-300 cursor-pointer">Зарегистрируйтесь</button></>
            ) : (
              <>Уже есть аккаунт? <button onClick={() => switchMode('login')} className="text-gold-400 hover:text-gold-300 cursor-pointer">Войдите</button></>
            )}
          </p>
        </div>

...
function isWikiMaster(article: WikiArticle): boolean {
  const cat = (article.fields?.category || '').toLowerCase();
  return cat.includes('загадочник') || /^##\s*загадочник/mi.test(article.content || '');
}

export function mergeRiddleMasters(wikiArticles: WikiArticle[]): RiddleMaster[] {
  const custom = wikiArticles
    .filter(a => a.section === 'riddles' && isWikiMaster(a))
    .map(wikiArticleToRiddleMaster);
  const wikiIds = new Set(custom.map(m => m.wikiId));
  const builtins = builtinMasters.filter(m => !wikiIds.has(m.wikiId));
  return [...builtins, ...custom].sort((a, b) => a.nameRu.localeCompare(b.nameRu, 'ru'));
}

export function mergeRiddleClues(wikiArticles: WikiArticle[]): RiddleClue[] {
  const custom = wikiArticles
    .filter(a => a.section === 'riddles' && !isWikiMaster(a))
    .map(wikiArticleToRiddleClue);
  const wikiIds = new Set(custom.map(c => c.wikiId));
  const builtins = builtinClues.filter(c => !wikiIds.has(c.wikiId));

...
export const GUILD_NAME_MAX_LENGTH = 40;
...
/** Название игровой гильдии для реестра */
export function validateGuildName(name: string): ValidationResult {
  const trimmed = name.trim();

  if (!trimmed) {
    return { valid: false, error: 'Название гильдии обязательно' };
  }

  if (trimmed.length < GUILD_NAME_MIN_LENGTH) {
    return { valid: false, error: `Название гильдии — минимум ${GUILD_NAME_MIN_LENGTH} символа` };
  }

  if (trimmed.length > GUILD_NAME_MAX_LENGTH) {
    return { valid: false, error: `Название гильдии — не длиннее ${GUILD_NAME_MAX_LENGTH} символов` };
  }

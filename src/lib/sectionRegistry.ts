import { WIKI_HUB_SECTIONS, CONTENT_SECTION_IDS, type WikiSectionMeta } from '../data/sections';
...
export function sanitizeSectionDefinition(raw: unknown): CustomSectionDefinition | null {
  ...
  if (!id || !/^[a-z0-9][a-z0-9-]*$/i.test(id)) return null;
...
export function createEmptySection(label: string): CustomSectionDefinition {
  ...
  return {
    ...
    categories: [{ id: 'Прочее', label: 'Прочее', icon: '✦' }],
  };
}

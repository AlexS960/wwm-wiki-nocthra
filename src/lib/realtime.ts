...
/** Подписка на изменения новостей */
export function subscribeSiteNews(onChange: () => void): () => void {
  const channel = getSupabase()
    .channel('site_news-realtime')
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'site_news' },
      () => onChange(),
    )
    .subscribe();
...
/** Подписка на тикеты поддержки */
export function subscribeSupportTickets(onChange: () => void): () => void {
  const channel = getSupabase()
    .channel('support_tickets-realtime')
    .on(
      'postgres_changes',
...
/** Подписка на версии гайдов */
export function subscribeGuideVersions(onChange: () => void): () => void {
  const channel = getSupabase()
    .channel('guide_versions-realtime')
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'guide_versions' },
      () => onChange(),
    )
    .subscribe();

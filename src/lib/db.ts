...
// ====== accounts ======
export async function dbInit() { await supabase.from('accounts').select('id').limit(1); }
...
export async function dbLoadSiteData<T>(key: string, fallback?: T): Promise<T> {
  ...
  if (data) { try { return JSON.parse(data.value) as T; } catch { return data.value as any; } }
  return fallback as T;
}

export async function dbSaveSiteData(key: string, value: unknown) {
  const s = JSON.stringify(value);
  await supabase.from('site_data').upsert({ key, value: s, updated_at: new Date().toISOString() }, { onConflict: 'key' });
}
...
export async function dbLoadProgress(userId: string): Promise<any> {
  ...
  if (data?.data) { try { return JSON.parse(data.data); } catch { return data.data; } }
  return null;
}

export async function dbSaveProgress(userId: string, data: any) {
  await supabase.from('user_progress').upsert({ user_id: userId, data: JSON.stringify(data), updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
}

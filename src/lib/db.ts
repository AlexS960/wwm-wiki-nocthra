...
import { isStaffChatRole } from './staffChat';
...
/** Аккаунты со служебными ролями (для чата команды). */
export async function dbListStaffAccounts(): Promise<DbAccount[]> {
  const roles = ['admin', 'guildmaster', 'editor', 'moderator'];
  const { data, error } = await getSupabase().from('accounts').select('*').in('role', roles);
  if (!error && data?.length) return data as DbAccount[];
  const all = await dbListAccounts();
  return all.filter(a => isStaffChatRole(a.role));
}

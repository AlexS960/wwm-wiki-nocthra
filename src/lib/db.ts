...
export async function dbSetOnline(userId: string): Promise<void> {
  try {
    if (await isOnline()) {
      const { data } = await supabase.from('site_data').select('data').eq('key', 'online_status').single();
      const statuses: Record<string, number> = (data?.data as Record<string, number>) || {};
      statuses[userId] = Date.now();
      await supabase.from('site_data').update({ data: statuses, updated_at: new Date().toISOString() }).eq('key', 'online_status');
    }
  } catch {}
}
...
export async function dbGetOnlineStatuses(): Promise<Record<string, number>> {
  try {
    if (await isOnline()) {
      const { data } = await supabase.from('site_data').select('data').eq('key', 'online_status').single();
      if (data?.data) return data.data as Record<string, number>;
    }
  } catch {}
  return {};
}
...
export async function dbInit(): Promise<void> {
  ...
  if (ok) {
    ...
    // Ensure online_status row exists
    try {
      const { data: osData } = await supabase.from('site_data').select('key').eq('key', 'online_status').single();
      if (!osData) {
        await supabase.from('site_data').insert({ key: 'online_status', data: {}, updated_at: new Date().toISOString() });
      }
    } catch {}

...
export async function fetchAiStatus(): Promise<DeepSeekStatus> {
  const data = await syncFetch<{ ai: DeepSeekStatus }>('/api/sync-content?action=ai-status');
  return data.ai;
}
...
export function buildSettingsPatch(
  ...
  const parsedContent = { ...(current.parsedContent || {}) };

...
import { dispatchCloseAllOverlays, CLOSE_ALL_OVERLAYS } from './lib/closeFloatPanels';
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      if (!target) return;
      if (target.closest('.overlay-panel, [role="dialog"], button, input, textarea, select, a, label')) return;
      closePanels();
    };
    ...
    const onWindowScroll = () => {
      ...
      closeScheduled = true;
      requestAnimationFrame(() => closePanels());

...
function AppContent() {
  ...
  if (currentPage === 'guides') {
    return (<div className="min-h-screen bg-ink-900 text-ink-100 page-enter"><Header {...headerProps} /><GuidesPage onBack={goBack} /><Footer />{modals}</div>);
  }
    ...
    <div className="min-h-screen bg-ink-900 text-ink-100">
      ...
      <main>
        ...
        {activeAnnouncements.length > 0 && (
          <div className="announcement-float">
            {activeAnnouncements.map(ann => (
              <div key={ann.id} className={`overflow-hidden py-2 px-4 rounded-full backdrop-blur-md shadow-lg ${
        ...
        {isAdmin() && (
          <div className="fixed left-4 z-40 flex flex-col gap-2" style={{ top: activeAnnouncements.length > 0 ? `${84 + activeAnnouncements.length * 40 + 8}px` : '92px' }}>
            <button onClick={() => onNavigate('admin')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-ink-900/85 backdrop-blur-md border border-purple-500/35 shadow-lg hover:bg-purple-500/10 hover:border-purple-500/55 transition-all cursor-pointer group w-fit">
              <div className="w-7 h-7 rounded-lg bg-purple-500/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                <Crown className="w-3.5 h-3.5 text-purple-400" />
              </div>
              <span className="text-purple-400 text-xs font-medium">Панель управления</span>
            </button>
  ...
  if (!isOpen) {
    return (
      <button onClick={() => setIsOpen(true)}
        className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-ink-900/85 backdrop-blur-md border border-orange-500/35 shadow-lg hover:bg-orange-500/10 hover:border-orange-500/55 transition-all cursor-pointer group w-fit">
        <div className="w-7 h-7 rounded-lg bg-orange-500/20 flex items-center justify-center group-hover:scale-110 transition-transform">
          <Megaphone className="w-3.5 h-3.5 text-orange-400" />
        </div>
        <span className="text-orange-400 text-xs font-medium">Объявления</span>
      </button>
    );
  }

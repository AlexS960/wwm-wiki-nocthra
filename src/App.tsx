...
import { dispatchCloseAllOverlays } from './lib/closeFloatPanels';
...
function AppContent() {
  const { siteSettings, canAccessAdminPanel, isLoading } = useAuth();
  ...
  const headerProps = {
    ...
    showAdminLink: canAccessAdminPanel(),
  };
    ...
    <>
      ...
      {showScrollTop && (
        <button
          type="button"
          onClick={scrollToTop}
          className="hover-glow-btn fixed bottom-20 right-6 z-40 bg-gold-400/20 backdrop-blur-sm border border-gold-400/40 text-gold-400 p-3 rounded-full shadow-lg hover:bg-gold-400/30 transition-all animate-fadeIn cursor-pointer"
        >
  ...
  if (currentPage !== 'main' && currentPage !== 'admin' && isSectionMaintenance && !canAccessAdminPanel()) {
    return (
      <PageShell headerProps={headerProps} modals={modals}>
        <MaintenancePage

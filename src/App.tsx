...
import { ArrowUp, Crown, Sun, Moon } from 'lucide-react';
...
import { isKnownPath, pageFromPath, pathFromPage } from './lib/appRoutes';
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    if (!fromUrl && !isKnownPath(pathname)) {
      window.history.replaceState({ page: 'main' }, '', '/');
      setCurrentPage('main');
    } else {
      const page = fromUrl || 'main';
  ...
  const modals = (
    <>
      ...
      <button
        type="button"
        onClick={toggleTheme}
        className="hover-glow-btn fixed bottom-20 right-16 z-40 bg-ink-900/90 border border-gold-400/40 text-gold-400 p-3 rounded-full shadow-lg hover:bg-gold-400/20 animate-fadeIn cursor-pointer"
        title={theme === 'auto' ? 'Авто тема' : theme === 'dark' ? 'Темная тема' : 'Светлая тема'}
      >
        {theme === 'auto' ? (

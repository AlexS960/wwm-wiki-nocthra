...
function normalizeDialogues(raw) {
  if (!raw?.dialogueLines?.length) return [];
  return raw.dialogueLines
    .map(l => {
      let role = l.role || 'npc';
      const textEn = (l.textEn || l.text || '').trim();
      const textRu = (l.textRu || textEn).trim();
      if (role === 'npc' && !textEn.startsWith('(')) {
        if (/^(To |In order|it taught|but of course|I |I'm )/i.test(textEn)) role = 'player';
        else if (/^[a-z]/.test(textEn) && textEn.length < 160
          && !/^(indeed|remarkable|thank you|wise|many |it is |such |do you|please|hmmm)/i.test(textEn)) {
          role = 'player';
        }
      }
      return { role, textEn, textRu };
    })
    .filter(l => l.textEn.length > 1);
}
...
const npcs = parsed.map(r => {
  const d = {
    befriendGuide: (dialogues[r.id]?.befriendGuide || '').trim(),
    dialogues: normalizeDialogues(dialogues[r.id]),
  };
  return {
    id: r.id,
    nameEn: r.nameEn,
    ...
    icon: icon(r.nameEn),
    befriendGuide: d.befriendGuide.slice(0, 2000),

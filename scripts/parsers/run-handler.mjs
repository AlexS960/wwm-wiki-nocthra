...
import { enrichPayloadWithDeepSeek } from './lib/deepseek.mjs';
...
export async function runSyncSection(section, opts = {}) {
  ...
  if (!dryRun && !result?.skipped && !result?.error) {
    ...
    if (payload && opts.useAi !== false) {
      try {
        const enriched = await enrichPayloadWithDeepSeek(section, payload, opts.onProgress);
        payload = enriched.payload;
        ai = enriched.ai;
      } catch (e) {
        ai = { used: false, error: e.message || 'DeepSeek error' };
      }
    }
  }

...
Браузер
   │
   ├─ https://wwm-wiki-nocthra.ru          → nginx → /var/www/wwm-wiki (SPA)
   ├─ https://wwm-wiki-nocthra.ru/api/sync-content → nginx → sync-api :3001
   └─ https://api.wwm-wiki-nocthra.ru      → nginx → Kong (Supabase Docker) :8000
                                              ├─ /rest/v1   PostgREST
                                              ├─ /realtime/v1
                                              └─ /storage/v1
```
...
Сайт: `wwm-wiki-nocthra.ru`, API: `api.wwm-wiki-nocthra.ru`.
...
# Домены уже заданы в файле: wwm-wiki-nocthra.ru, api.wwm-wiki-nocthra.ru
...
Или вручную: `scp -r dist/* deploy@62.109.19.21:/var/www/wwm-wiki/`
